...
class Categorias extends CI_Controller {
	...
	public function getCategorias()
	{
        ...
        $totalData = $this->Categorias_model->allcategorias_count();
            
        $totalFiltered = $totalData; 
            
        if(empty($this->input->post('search')['value']))
        {            
            $categorias = $this->Categorias_model->allcategorias($limit,$start,$order,$dir);
        }
        else {
            $search = $this->input->post('search')['value']; 

            $categorias =  $this->Categorias_model->categorias_search($limit,$start,$search,$order,$dir);

            $totalFiltered = $this->Categorias_model->categorias_search_count($search);
        }
        ...
        if(!empty($categorias))
        {
            foreach ($categorias as $categoria)
            {
                ...
                $nestedData['id'] = $categoria->id;
                $nestedData['nombre'] = $categoria->nombre;
                $nestedData['descripcion'] = $categoria->descripcion;
