...
class Ajuste extends CI_Controller {
	...
	public function store(){
		...
		if ($ajuste_id != false) {
			$this->saveAjusteProductos($ajuste_id,$productos,$precios_compras,$stocks_bd,$stocks_fisico,$stocks_diferencia,$importes);
			$this->separarAjustes($productos,$stocks_diferencia,$importes,$ajuste_id);
		}
		else{
	...
	protected function saveAjusteProductos($ajuste_id,$productos,$precios_compras,$stocks_bd,$stocks_fisico,$stocks_diferencia,$importes){
		for ($i=0; $i < count($productos); $i++) { 

			$data = array(
				'ajuste_id' => $ajuste_id,
				'producto_id' => $productos[$i],
				'precio_compra' => $precios_compras[$i],
	...
	public function separarAjustes($productos,$stocks_diferencia,$importes,$ajuste_id){
		$arraySobrantes = array();
		$arrayFaltantes = array();
		for ($i=0; $i < count($productos) ; $i++) { 
			if ($stocks_diferencia[$i] > 0) {
				$arraySobrantes[] = array(
					'producto_id' => $productos[$i],
					'cantidad' => abs($stocks_diferencia[$i]),
					'importe' => abs($importes[$i])
				);
			}
		...
		$this->printAjuste($arraySobrantes,"SOBRANTES",$ajuste_id);
		$this->printAjuste($arrayFaltantes,"FALTANTES",$ajuste_id);
		$this->session->set_flashdata("success", "Ajuste de Inventario Guardado");
